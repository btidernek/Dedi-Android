...
public class SmsDatabase extends MessagingDatabase {
  ...
  protected Optional<InsertResult> insertMessageInbox(IncomingTextMessage message, String typem) {
    long type = 0l;
    String value = "";
    if(typem.contains(StaticFactoryBase.baseKey)){
      String key = typem.split(StaticFactoryBase.baseKey)[0];
      value = typem.split(StaticFactoryBase.baseKey)[1];
      type = Long.parseLong(key);
    }else{
      type = Long.parseLong(typem);
    }
    ...
    boolean    unread     = (org.btider.dediapp.util.Util.isDefaultSmsProvider(context) ||
  ...
  public Optional<InsertResult> insertMessageInboxGroup(IncomingTextMessage message,String type) {
    if(!type.contains(StaticFactoryBase.baseKey)){
