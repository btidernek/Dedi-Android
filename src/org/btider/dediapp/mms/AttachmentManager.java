...
import org.btider.dediapp.MediaPreviewActivity;
import org.btider.dediapp.R;
import org.btider.dediapp.attachments.Attachment;
import org.btider.dediapp.components.AudioView;
import org.btider.dediapp.components.DocumentView;
import org.btider.dediapp.components.RemovableEditableMediaView;
import org.btider.dediapp.components.ThumbnailView;
import org.btider.dediapp.components.location.SignalMapView;
...
import gov.btider.multiselectgallary.GalleryConfig;
    ...
    new AsyncTask<Void, Void, Slide>() {
      ...
      protected void onPostExecute(@Nullable final Slide slide) {
        if (slide == null) {
          ...
          attachmentViewStub.get().setVisibility(View.GONE);
          //TODO : içerik değiştiğinden dolayı bu text artık gereksiz.
          String boyut = "";
          if(getBooleanPreference(context, PREF_MEDIA_SIZE, false)){
            Toast.makeText(context,
                    String.format(context.getString(R.string.ConversationActivity_attachment_exceeds_size_limits_low),boyut),
                    Toast.LENGTH_SHORT).show();
          }else{
            Toast.makeText(context,
                    String.format(context.getString(R.string.ConversationActivity_attachment_exceeds_size_limits),boyut),
                    Toast.LENGTH_SHORT).show();
          }
//          Toast.makeText(context,
//                        String.format(context.getString(R.string.ConversationActivity_attachment_exceeds_size_limits),boyut),
//                        Toast.LENGTH_SHORT).show();
          ...
          attachmentListener.onAttachmentChanged();
          ConversationActivity.conttimer = true;
        }
      }
  ...
  public static void selectGallery(Activity activity, int requestCode, int mediaSize) {
    Permissions.with(activity)
               .request(Manifest.permission.WRITE_EXTERNAL_STORAGE)
               .ifNecessary()
               .withPermanentDenialDialog(activity.getString(R.string.AttachmentManager_signal_requires_the_external_storage_permission_in_order_to_attach_photos_videos_or_audio))
               .onAllGranted(() -> selectMediaTypeIMAGE(activity, "image/*", new String[] {"image/*", "video/*"}, requestCode,mediaSize))
               .execute();
  }
  ...
  private static void selectMediaTypeIMAGE(Activity activity, @NonNull String type, @Nullable String[] extraMimeType, int requestCode,int medyaSize) {

    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
      try {
        GalleryConfig config = new GalleryConfig.Build()
                .limitPickPhoto(medyaSize)
                .singlePhoto(false)
                .hintOfPick(activity.getString(R.string.ConversationActivity_message_is_send_max,medyaSize+""))
                //.filterMimeTypes(new String[]{"image/jpeg","image/png","image/jpg"})
                .build();
  ...
  public enum MediaType {
    ...
    public Bitmap getResizedBitmap(Bitmap bm, double newWidth, double newHeight) {
