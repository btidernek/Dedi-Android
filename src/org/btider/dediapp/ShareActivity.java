    ...
    implements ContactSelectionListFragment.OnContactSelectedListener, SwipeRefreshLayout.OnRefreshListener
{
  ...
  private ArrayList<Uri>                          resolvedExtra;
  ...
  private void initializeMedia() {
    ...
    if(list == null){
      ...
      list.add(streamExtra);
    }

    for(Parcelable p : list){
      uris.add((Uri) p);
      mineTypes.add(getMimeType((Uri) p));
    }
    Uri streamExtra = (Uri) list.get(0);
    if (streamExtra != null && PartAuthority.isLocalUri(streamExtra)) {
      ...
      new ResolveMediaTaskMulti(context).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, uris);
    }
  ...
  private Intent getBaseShareIntent(final @NonNull Class<?> target) {
    ...
    //if (resolvedExtra != null) intent.setDataAndType(resolvedExtra.get(0), mimeType);
    int i = 0;
    if(resolvedExtra != null && resolvedExtra.size()>1){
      for(Uri uri : resolvedExtra){
        if(i != 0){
          if (uri != null) intent.putExtra("image"+i,uri);
        }
        i++;
      }
    }
    intent.putExtra("image_size",resolvedExtra.size());
    intent.putExtra("media_types",mineTypes);
    intent.putExtra("media_uris",uris);
  ...
  public void onContactSelected(String number) {
    ...
    System.out.println("------:how:"+existingThread);
  ...
  @SuppressLint("StaticFieldLeak")
  private class ResolveMediaTaskMulti extends AsyncTask<ArrayList<Uri>, Void, ArrayList<Uri>> {
    private final Context context;

    ResolveMediaTaskMulti(Context context) {
      this.context = context;
    }

    ArrayList<Uri> urism = new ArrayList<>();

    @Override
    protected ArrayList<Uri> doInBackground(ArrayList<Uri> ... uriss) {
      try {

        for(ArrayList<Uri> uris2 : uriss){
          for(Uri uris : uris2){
            if(uris != null){
              InputStream inputStream;
              ...
              Cursor cursor   = getContentResolver().query(uris, new String[] {OpenableColumns.DISPLAY_NAME, OpenableColumns.SIZE}, null, null, null);
              String fileName = null;
              Long   fileSize = null;
              ...
              urism.add(PersistentBlobProvider.getInstance(context).create(context, inputStream, mimeType, fileName, fileSize));
            }
          }
        }
