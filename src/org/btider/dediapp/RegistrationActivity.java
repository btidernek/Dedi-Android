...
import org.btider.dediapp.animation.AnimationCompleteListener;
import org.btider.dediapp.backup.FullBackupBase;
import org.btider.dediapp.backup.FullBackupImporter;
import org.btider.dediapp.components.registration.CallMeCountDownView;
import org.btider.dediapp.components.registration.VerificationCodeView;
import org.btider.dediapp.components.registration.VerificationPinKeyboard;
...
import org.btider.dediapp.lock.RegistrationLockReminders;
...
import org.btider.dediapp.util.PlayServicesUtil;
...
import org.btider.dediapp.lock.RegistrationLockReminders;
import org.btider.dediapp.permissions.Permissions;
import org.btider.dediapp.permissions.RationaleDialog;
import org.btider.dediapp.push.AccountManagerFactory;
import org.btider.dediapp.service.DirectoryRefreshListener;
import org.btider.dediapp.service.RotateSignedPreKeyListener;
import org.btider.dediapp.util.BackupUtil;
...
import org.btider.dediapp.util.Dialogs;
import org.btider.dediapp.util.PlayServicesUtil;
import org.btider.dediapp.util.PlayServicesUtil.PlayServicesStatus;
import org.btider.dediapp.util.ServiceUtil;
import org.btider.dediapp.util.TextSecurePreferences;
import org.btider.dediapp.util.Util;
import org.btider.dediapp.util.concurrent.AssertedSuccessListener;
...
public class RegistrationActivity extends BaseActionBarActivity implements VerificationCodeView.OnCodeEnteredListener {
  ...
  public static final  String CHALLENGE_EVENT           = "org.btider.dediapp.CHALLENGE_EVENT";
  ...
  private void handleRegisterWithPermissions() {
    ...
    PlayServicesUtil.PlayServicesStatus gcmStatus = PlayServicesUtil.getPlayServicesStatus(this);

    if (gcmStatus == PlayServicesUtil.PlayServicesStatus.SUCCESS) {
      handleRequestVerification(e164number, true);
    } else if (gcmStatus == PlayServicesUtil.PlayServicesStatus.MISSING) {
      handlePromptForNoPlayServices(e164number);
    } else if (gcmStatus == PlayServicesUtil.PlayServicesStatus.NEEDS_UPDATE) {
  ...
  private void handleRequestVerification(@NonNull String e164number, boolean gcmSupported) {
    ...
    new AsyncTask<Void, Void, Pair<String, Optional<String>>> () {
      ...
      protected void onPostExecute(@Nullable Pair<String, Optional<String>> result) {
        ...
        displayVerificationView(e164number, 120);
      }
  ...
  private void handlePhoneCallRequest() {
    if (registrationState.state == RegistrationState.State.VERIFYING) {
      callMeCountDownView.startCountDown(300,this);
  ...
  private void displayVerificationView(@NonNull String e164number, int callCountdown) {
    ...
    this.callMeCountDownView.startCountDown(callCountdown,this);
  }
