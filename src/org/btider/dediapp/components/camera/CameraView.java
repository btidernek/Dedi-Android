...
package org.btider.dediapp.components.camera;
...
import org.btider.dediapp.toolscustom.EnregistrementVideoStack;
    ...
    public CameraView(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (attrs != null) {
            ...
            if (camera != -1) cameraId = camera;
                ...
                cameraId = TextSecurePreferences.getDirectCaptureCameraId(context);
...
//    if (camera.isPresent()) startPreview(camera.get().getParameters());
    ...
    private EnregistrementVideoStack eVideoStack = null;
...
//      }
    ...
    public void takeVideo(ImageButton shutterButton, ImageButton swapCameraButton, QuickAttachmentDrawer.DrawerState drawerState, ImageButton fullscreenButton, TextView timeClock) {
        ...
        if (!isVideoRecord) {
            ...
            if (eVideoStack == null) {
                ...
                eVideoStack = new EnregistrementVideoStack(getActivity(), surface, previewSize, rotation, cameraId, timeClock);

            }

            if (eVideoStack != null) {
                swapCameraButton.setVisibility(GONE);
                eVideoStack.startRecort();
            }
        } else {
            isVideoRecord = false;
            swapCameraButton.setVisibility(VISIBLE);
            animation.cancel();
            animation.setAnimationListener(new Animation.AnimationListener() {
                @Override
                public void onAnimationStart(Animation animation) {
            ...
            if (eVideoStack != null) {
                byte[] video = eVideoStack.stopRecort();
                for (CameraViewListener listener : listeners) {
                    listener.onVideoCapture(video);
                }
                eVideoStack = null;
            }
        }
    ...
    public void takePicture(final Rect previewRect, ImageButton swapCameraButton) {
        if (!isVideoRecord) {
            if (!camera.isPresent() || camera.get().getParameters() == null) {
                Log.w(TAG, "camera not in capture-ready state");
                return;
    ...
    public interface CameraViewListener {
        ...
        void onVideoCapture(@NonNull final byte[] imageBytes);
