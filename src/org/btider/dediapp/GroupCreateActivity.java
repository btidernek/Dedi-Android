        ...
        PushRecipientsPanel.RecipientsPanelChangedListener
{
  ...
  private void handleGroupCreate() {
    ...
    if(getGroupName().equals("")){
      Log.i(TAG, getString(R.string.GroupCreateActivity_group_name_null));
      Toast.makeText(getApplicationContext(), R.string.GroupCreateActivity_group_name_null, Toast.LENGTH_SHORT).show();
      return;
    }
    if (isSignalGroup()) {
  ...
  private void handleGroupUpdate() {
    new UpdateSignalGroupTask(this, groupToUpdate.get().id, avatarBmp,
                              getGroupName(), getAdapter().getRecipients(),true).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
  }
  ...
  public void UpdateGroupTask(Recipient recipient){
    Set<Recipient> recipients = getAdapter().getRecipients();
    recipients.remove(recipient);
    new UpdateSignalGroupTask(this, groupToUpdate.get().id, avatarBmp,
            getGroupName(), recipients,false).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
  }

  public static class UpdateSignalGroupTask extends SignalGroupTask {
    private String groupId;
    private boolean updateDbBeforeSend;

    public UpdateSignalGroupTask(GroupCreateActivity activity, String groupId,
                                 Bitmap avatar, String name, Set<Recipient> members, boolean updateDbBeforeSend)
    {
      super(activity, avatar, name, members);
      this.groupId = groupId;
      this.updateDbBeforeSend = updateDbBeforeSend;
    }

    @Override
    protected Optional<GroupManager.GroupActionResult> doInBackground(Void... aVoid) {
      try {
        return Optional.of(GroupManager.updateGroup(activity, groupId, members, avatar, name,updateDbBeforeSend));
  ...
  private static class FillExistingGroupInfoAsyncTask extends ProgressDialogAsyncTask<String,Void,Optional<GroupData>> {
    ...
    protected void onPostExecute(Optional<GroupData> group) {
      ...
      if (group.isPresent() && !activity.isFinishing()) {
        ...
        SelectedRecipientsAdapter adapter = new SelectedRecipientsAdapter(activity, group.get().recipients,group.get().id);
